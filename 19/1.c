...
bool isPatternPossible(char* pattern, char** towels, int towelCount, char*** patternCache_p, bool** possibleCache_p, int* cacheLen_p, int* cacheCapacity_p);
...
int main(int argc, char** argv) {
    ...
    bool* possibleCache = malloc(sizeof *possibleCache);

    int result = 0;
    for (int i = 0; i < patternCount; i++) {
        cacheLen = 0; // Reset cache each time as old cache is useless
        result += isPatternPossible(patterns[i], towels, towelCount, &patternCache, &possibleCache, &cacheLen, &cacheCapacity);
        free(patterns[i]);
    }
...
bool isPatternPossible(char* pattern, char** towels, int towelCount, char*** patternCache_p, bool** possibleCache_p, int* cacheLen_p, int* cacheCapacity_p) {
    ...
    for (int i = 0; i < *cacheLen_p; i++) if (pattern == (*patternCache_p)[i]) return (*possibleCache_p)[i];
    ...
    for (int i = 0; i < towelCount; i++) {
        ...
        if (isPatternPossible(pattern + len, towels, towelCount, patternCache_p, possibleCache_p, cacheLen_p, cacheCapacity_p)) {
            possible = true;
            break;
        }
    }
    ...
    if (*cacheLen_p == *cacheCapacity_p) {
        ...
        *possibleCache_p = realloc(*possibleCache_p, *cacheCapacity_p * sizeof **possibleCache_p);
    }
    (*patternCache_p)[*cacheLen_p] = pattern;
    (*possibleCache_p)[*cacheLen_p] = possible;
    (*cacheLen_p)++;
