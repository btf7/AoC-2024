...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < reportCount; i++) {
        if (isReportSafe(reports[i])) {
            ...
            // Brute force search which number to remove
            for (int j = 0; j < reports[i].levelCount; j++) {
                struct report newReport;
                newReport.levelCount = reports[i].levelCount - 1;

                for (int k = 0; k < j; k++) {
                    newReport.levels[k] = reports[i].levels[k];
                }
                for (int k = j; k < newReport.levelCount; k++) {
                    newReport.levels[k] = reports[i].levels[k + 1];
                }
